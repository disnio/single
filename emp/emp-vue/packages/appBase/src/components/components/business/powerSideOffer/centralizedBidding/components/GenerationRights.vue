 ...
 * @最新修改内容: #2353   修改查看时报价段显示
 * @LastEditTime : 2020-01-16 20:40:25
 -->
<template>
  <div class="app-container calendar-list-container">
    ...
    >
      ...
      <!-- 2019-10-30   #2340  康如涛 修改查看时报价段显示 start-->
      ...
      <el-table-column align="center" label="操作" width="100" v-if="!dialogInfo.notEdit">
        <template slot-scope="scope">
          <el-button
...
export default {
  ...
  data() {
    return {
      ...
      unitList: [],
       loading:false,
      eleEnterpriseList:[],//#3545 2020-01-13 张亮三 用电企业数据数组
  ...
  methods: {
    ...
    checkTradingUnit(data) {
      ...
      this.tableList.forEach(item => {
        if (data.index > item.index) {
          if (data.quotationStall == item.quotationStall) {
            isErr = true;
          }
        }
      });
    ...
    checkTab(data) {
      ...
      for (let i = 0, l = this.tableList.length; i < l; i++) {
        ...
        if (
          ...
          typeof this.tableList[i].unit == "undefined"
        ) {
    ...
    handleDelete(row) {
      //    this.delRowData(row);
      //    this.tableList.forEach(item=>{
      //      console.log(item.detailId+"==="+item.index);
      //    })
