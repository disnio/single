 ...
 * @最新修改内容: 
 * @LastEditTime: 2020-08-25 14:25:30
 -->
<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table
      class="input-table"
      :data="importTabList"
      v-if="this.tradingVariety != 'jypz09'"
    ...
    >
      ...
      <el-table-column align="center" label="单位名称">
        <template slot-scope="scope">
          <el-tooltip
            v-if="scope.row.sdOrgNameStatus=='ipt-err-true'"
            class="item"
            effect="dark"
            :content="scope.row.sdOrgNameStatusMsg"
            placement="top"
          >
            <span :class="scope.row.sdOrgNameStatus">{{scope.row.sdOrgName}}</span>
          </el-tooltip>
          <span v-else>{{scope.row.sdOrgName}}</span>
        </template>
      </el-table-column>
      <!-- 2020-06-04 #4151  qinhz 导入解析-交易结果-售电-交易品种为省内直接交易-新增购电侧交易单元字段 start-->
      ...
      <!-- 2020-06-04 #4151  qinhz 导入解析-交易结果-售电-交易品种为省内直接交易-新增购电侧交易单元字段 end-->
       <el-table-column align="center" label="交易线路" >
        <template slot-scope="scope">
          <div class="txt-left">
            {{scope.row.tradingLineName}}
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="交易单元">
        <template slot-scope="scope">
          <el-tooltip
            v-if="scope.row.tradingUnitNameStatus=='ipt-err-true'"
            class="item"
            effect="dark"
          ...
          >
            ...
            >{{scope.row.tradingUnitName}}</span>
          </el-tooltip>
          <span v-else>{{scope.row.tradingUnitName}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="数据异常原因" width="600px"  v-if="isErrInfo">
        <template slot-scope="scope">
          <el-tooltip
            v-if="scope.row.remarkStatus=='ipt-err-true'"
            class="item"
            effect="dark"
            :content="scope.row.remarkMsg"
            placement="top"
          >
            <span :class="scope.row.remarkStatus">{{scope.row.remark}}</span>
          </el-tooltip>
    ...
    >
      ...
      <el-table-column align="center" label="合同名称">
        <template slot-scope="scope">
          <el-tooltip
            v-if="scope.row.contractNameStatus=='ipt-err-true'"
            class="item"
            effect="dark"
            :content="scope.row.contractNameStatusMsg"
            placement="top"
          >
            <span :class="scope.row.contractNameStatus">{{scope.row.contractName}}</span>
          </el-tooltip>
      ...
      <el-table-column align="center" label="类型">
        <template slot-scope="scope">
          <el-tooltip
            v-if="scope.row.typeNameStatus=='ipt-err-true'"
            class="item"
            effect="dark"
            :content="scope.row.typeNameStatusMsg"
            placement="top"
          >
            <span
              :class="scope.row.typeNameStatus"
      ...
      <el-table-column align="center" label="售电公司">
        <template slot-scope="scope">
          ...
          >
            ...
            >{{scope.row.sdOrgName}}</span>
          </el-tooltip>
          <span v-else>{{scope.row.sdOrgName}}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="出/受让售电企业">
        <template slot-scope="scope">
          <el-tooltip
            v-if="scope.row.transSdOrgNameStatus=='ipt-err-true'"
            class="item"
            effect="dark"
            :content="scope.row.transSdOrgNameStatusMsg"
            placement="top"
          >
            <span
              :class="scope.row.transSdOrgNameStatus"
            >{{scope.row.transSdOrgName}}</span>
          </el-tooltip>
          <span v-else>{{scope.row.transSdOrgName}}</span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    //导出异常数据
    exportData() {
      exportDataErrData("jySellNotice", this.resultId);
    },
    //保存导入
    createImport(type) {
      ...
      let parame = {};
      parame.description = "jySellNotice";
