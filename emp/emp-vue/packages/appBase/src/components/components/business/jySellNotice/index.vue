  ...
  <div class="app-container calendar-list-container">
    <div class="filter-container f-ml10 f-mt10">
      ...
      <el-form :inline="true" :model="listQuery" class="demo-form-inline" ref="listQueryForm" style="padding-bottom:15px">
        ...
        <el-form-item label="交易期次">
          <el-date-picker
            v-if="listQuery['tradingCycle'] != 'jyzq01'&&listQuery['tradingCycle'] != 'jyzq02'&&listQuery['tradingCycle'] != 'jyzq03'&&listQuery['tradingCycle'] != 'jyzq04'&&listQuery['tradingCycle'] != 'jyzq05'"
            type="year"
            class="w180"
            :clearable="true"
            placeholder="选择年"
          ...
          <el-date-picker
            ...
            class="w180"
          ></el-date-picker>
          <span v-if="listQuery['tradingCycle'] == 'jyzq02'">
            <el-select
              style="width:60px;"
              popper-class="select"
              @change="setTradeIssure"
    ...
    <el-row>
      ...
      <el-button-group class="f-mb10">
        <!-- <el-button
          v-if="jySellNoticeManager_btn_add"
          class="filter-item"
          type="primary"
          icon="el-icon-plus"
          @click="handleCreate"
        ...
        >
        <el-button
          v-if="jySellNoticeManager_btn_del"
          type="primary"
          icon="el-icon-delete"
    ...
    >
     <!-- #4376 去掉选择框 -->
      ...
      <el-table-column prop="index" label="序号" align="center" width="50px"></el-table-column>
      ...
      <el-table-column align="center" label="成交电价">
        <template slot-scope="scope">
          <div class="txt-right">{{ scope.row.dealPrice }}</div>
        </template>
      </el-table-column>
      ...
      <el-table-column fixed="right" align="center" label="操作" width="150">
        <template slot-scope="scope">
          <!-- <el-button size="small" type="text" @click="handleUpdate(scope.row)">查看
          </el-button>-->
          <!-- #2716 2019-11-19 张亮三 查看按钮只有在已保存或已提交状态下显示 -->
          <el-button
            v-if="scope.row.statusName=='已提交' || scope.row.statusName=='已保存'"
            size="small"
            type="text"
            @click="handleUpdate(scope.row, 'view')"
          >查看</el-button>
          <!-- #2716 2019-11-19 张亮三 结果维护按钮只有在已保存或者初始化时显示 -->
          <el-button
            v-if="btn.jySellNoticeManager_btn_edit && scope.row.statusName=='已保存' ||
            (scope.row.statusName==''|| typeof scope.row.statusName=='undefined')"
            size="small"
            type="text"
          ...
          <el-button
            v-if="btn.jySellNoticeManager_btn_no_deal&&  (scope.row.statusName==''|| typeof scope.row.statusName=='undefined')"
            size="small"
            type="text"
          ...
          <el-button
            v-if="btn.jySellNoticeManager_btn_no_modify && scope.row.statusName=='已提交'"
            size="small"
            type="text"
...
export default {
  ...
  data() {
    return {
      ...
      dialogStatus: "",
      btn: {
        jySellNoticeManager_btn_edit: false,
        jySellNoticeManager_btn_del: false,
        jySellNoticeManager_btn_add: false,
        jySellNoticeManager_btn_export: false,
        jySellNoticeManager_btn_import: false,
        jySellNoticeManager_btn_save: false,
        jySellNoticeManager_btn_submit: false,
        //#2716 2019-11-19 张亮三 增加未成交资源按钮 start
        jySellNoticeManager_btn_no_deal: false,
        //#2716 2019-11-19 张亮三 增加未成交资源按钮 end
        //#2873 2019-12-11 秦侯珍 增加修改资源按钮 start
        jySellNoticeManager_btn_no_modify: false
        //#2873 2019-12-11 秦侯珍 增加修改资源按钮 start
      },
      objcetData: {},
      textMap: {
  ...
  created() {
    ...
    this.init();
    this.getList();
    this.btn.jySellNoticeManager_btn_edit = this.elements[
      "jySellNoticeManager:btn_edit"
    ];
    this.btn.jySellNoticeManager_btn_del = this.elements[
      "jySellNoticeManager:btn_del"
    ];
    this.btn.jySellNoticeManager_btn_add = this.elements[
      "jySellNoticeManager:btn_add"
    ];
    this.btn.jySellNoticeManager_btn_export = this.elements[
      "jySellNoticeManager:btn_export"
    ];
    this.btn.jySellNoticeManager_btn_import = this.elements[
      "jySellNoticeManager:btn_import"
    ];
    this.btn.jySellNoticeManager_btn_save = this.elements[
      "jySellNoticeManager:btn_save"
    ];
    this.btn.jySellNoticeManager_btn_submit = this.elements[
      "jySellNoticeManager:btn_submit"
    ];
    //#2716 2019-11-19 张亮三 增加未成交资源按钮 start
    this.btn.jySellNoticeManager_btn_no_deal = this.elements[
      "jySellNoticeManager:btn_no_deal"
    ];
    //#2716 2019-11-19 张亮三 增加未成交资源按钮 end

    //#2873 2019-12-11 秦侯珍 增加修改资源按钮 start
    this.btn.jySellNoticeManager_btn_no_modify = this.elements[
      "jySellNoticeManager:btn_no_modify"
    ];
  ...
  methods: {
    ...
    handleUpdate(row, stauts) {
      ...
      // if (
      //   row.tradingCenterId == null ||
      //   row.tradingCenterId == "" ||
      //   typeof row.tradingCenterId == "undefined"
      // ) {
      ...
      //   });
      // }
      // #3403 2019-01-02 张亮三 解决报价方式不跟主列表报价方式显示不一样问题 start
      if (row.quotationMethod == "bjfs01") {
         // #3403 2019-01-02 张亮三 解决报价方式不跟主列表报价方式显示不一样问题 end
            this.electricityPriceStyle = false;
          } else {
            this.electricityPriceStyle = true;
          }
      //2019-12-6 #2933 张朋 根据编码获取发布时间
      pageList(row.jyTradingResultsId).then(res => {
        this.objcetData.noticeDate = res.data.noticeDate;
