 ...
 * @最新修改内容: 
 * @LastEditTime: 2019-12-10 17:21:17
 -->
<template>
          ...
          <div style=" margin-top: 20px; ">
            <el-row :gutter="5">
              <el-col :span="4">
                <div>
                  <div class="title-1">
                     <!-- 2019-11-07 #2601 康如涛 修改“预计成交总电量”为“申报总电量”  start -->
                    <span>申报总电量</span>
                       <!-- 2019-11-07 #2601 康如涛 修改“预计成交总电量”为“申报总电量”  end -->
                  </div>
                  <div class="data-info">
                ...
                <span v-else>
                  <span v-if="scope.row.state=='input'">
                      <el-input-number
                        ...
                        @change="changeInput"
                      ></el-input-number>
                    <el-input-number  v-else v-model="scope.row[item.id]"  @change="changeInput"></el-input-number>
                  </span>
                  <span v-else>
                    {{scope.row[item.id]}}
                  </span>
...
<script>
import { fdBenefitCalculationObj ,
getFdBilateralNegotiationEstimating } from "@common/api/business/solutionBenefitCalculation/index";
...
export default {
  ...
  methods: {
    ...
    init(data, schemeNameCode) {
      ...
      getFdBilateralNegotiationEstimating(parme).then(res => {
        // console.log(res);
        ...
        if (res.rel) {
          ...
          // ======================
               /** 2019-11-07 #2601 康如涛 解决方案效益测算-预计成交电量不显示  start */
            // this.mainData.totalElectricity.data =
            // res.data.expectedDealElectricityCount;
          this.mainData.totalElectricity.data =
            res.data.declarationElectricityCount;
           /** 2019-11-07 #2601 康如涛 解决方案效益测算-预计成交电量不显示 end */
