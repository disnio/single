  ...
  <div>
    ...
    >
      ...
      <div style="overflow:hidden">
        <el-button-group style="float: left; margin-bottom: 8px">
          ...
          <el-button
            ...
            @click="handleLowDown"
          >下 发</el-button>
          <el-button class="submit-btn" @click="handleTipsBtn">提示信息</el-button>
          <el-button class="filter-item" type="primary" @click="handleConfirmOver">确认完毕</el-button>
          <el-button class="submit-btn" @click="handleConfirm">确 认</el-button>
          <el-button class="filter-item" type="primary" @click="handleReport">生成上报模板</el-button>
        </el-button-group>
      ...
      >
        ...
        <el-table-column :label="`${ agoMonth } 偏差预测 `" align="center" width="150">
          <template slot-scope="scope">
            <el-input
              type="number"
              :disabled=" scope.row.stateCode== '6'"
              v-model="scope.row.lastMonthDeviationForecast"
            ></el-input>
          </template>
        </el-table-column>

        <el-table-column :label="`${ agoMonth } 偏差率预测`" align="center" width="150">
          <template slot-scope="scope">
            <el-input
              type="number"
              :disabled=" scope.row.stateCode== '6'"
              v-model="scope.row.lastMonthDeviationRateForecast"
            ></el-input>
          </template>
        </el-table-column>

        <el-table-column label="近期偏差统计" align="center" width="120">
          <template slot-scope="scope">
            <el-input
              type="number"
              :disabled=" scope.row.stateCode== '6'"
              v-model="scope.row.recentlyDeviation"
            ></el-input>
          </template>
        </el-table-column>
        ...
        <el-table-column label="上年月度最大用电量" align="center" width="120">
          <template slot-scope="scope">
            <el-input
              :disabled=" scope.row.stateCode== '6'"
              type="number"
              v-model="scope.row.lastYearMostUsedElectricity"
            ></el-input>
          </template>
        </el-table-column>

        <el-table-column label="本月申报电量" align="center" width="120">
          <template slot-scope="scope">
            <el-input
              @change="blurInput(scope.row, scope.$index)"
              type="number"
              v-model.number="scope.row.declarationElectricity"
              :disabled=" scope.row.stateCode== '6'"
            ></el-input>
          </template>
        </el-table-column>

        <el-table-column label="申报建议调整" align="center" width="120">
          <template slot-scope="scope">
            <el-input
              @change="blurInput(scope.row, scope.$index)"
              type="number"
              v-model.number="scope.row.recommendedAdjustElectricity"
              :disabled=" scope.row.stateCode== '6'"
            ></el-input>
          </template>
        </el-table-column>

        <el-table-column label="调整后申报电量" align="center" width="120">
          <template slot-scope="scope">
            <el-input disabled v-model="scope.row.adjustedElectricity"></el-input>
          </template>
        </el-table-column>

        <el-table-column label="备注" align="center" width="180">
          <template slot-scope="scope">
            <!-- <el-input type="textarea" autosize v-model="scope.row.name"></el-input> -->
            <el-input :disabled=" scope.row.stateCode== '6'" v-model="scope.row.description"></el-input>
          </template>
        </el-table-column>

        <el-table-column label="申报状态" prop="stateName" align="center"></el-table-column>

        <el-table-column label="申报日期" prop="declarationDate" align="center" width="160">
          <template slot-scope="scope">
            <span>
              <el-date-picker
                :disabled=" scope.row.stateCode== '6'"
        ...
        <el-table-column align="center" label="操作" width="150" fixed="right">
          <template slot-scope="scope">
            <div v-if="scope.row.stateCode != '6'">
              <el-button size="small" type="text" @click="handleUpdate(scope.row, scope.$index)">确认</el-button>
              <el-button size="small" type="text" @click="handleReject(scope.row)">驳回</el-button>
            </div>
            <div v-else>
              <el-button size="small" type="text" @click="handleCancelConfirm(scope.row)">取消确认</el-button>
            </div>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 导入 弹窗 start -->
    <import-dialog ref="importDialog" :linedata="linedata" @fetchId="fetchId" />
    <!-- 导入 弹窗 end -->
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button 
        v-if="state == '新增'"
        class="save-btn" :loading="handleClickTipsLoad" @click="handleClickSave">保 存</el-button>
        <el-button class="cancel-btn" @click="tipsDialogVisible = false">关 闭</el-button>
      </div>
    </el-dialog>
    <!-- 提示信息弹窗  end -->
...
export default {
  ...
  data() {
    return {
      ...
      handleClickSaveLoad: false, //
      handleClickTipsLoad: false, //提示信息保存loading
      formInline: {
        page: 1,
        limit: 10,
        declarationTime: "",
  ...
  methods: {
    ...
    // 导入解析弹窗
    async showImportResult() {
      if(this.fileID){
        let res = await isOver(this.fileID)
        if(res.data.isanalyzed == '1'){
    ...
    async handleClickSave() {
      this.handleClickTipsLoad = true;
      let param = {
        declarationId: this.formInline.declarationId,
        tips: this.tipsText
      };
      let res = await putTips(this.formInline.declarationId, param);
      this.handleClickTipsLoad = false;
      if (res.rel) {
  ...
  watch: {
    // 监听 是否当前页全部确认 来切换 保存按钮的显示隐藏 状态
    tableData(val, oldVal) {
      let flag = val.find(item => {
        return item.stateCode != "6";
      });
      if (flag) {
        this.allConfimFlag = false;
      } else {
        this.allConfimFlag = true;
      }
    },
    deep: true,
    immediate: true
