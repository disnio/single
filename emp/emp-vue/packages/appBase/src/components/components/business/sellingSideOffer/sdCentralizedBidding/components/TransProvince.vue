<template>
  ...
  <div class="app-container calendar-list-container">
     <!-- 李志佳 2019年11月27日15:35:04 只在月季数据中显示-->
    <!-- <el-row  v-if="this.announcementInfoData.tradeCycleCode=='jyzq03'">
      <el-col :span="6">
        <div>跨区月竞交易电量:{{totalMonthBidOutprovElectricity}} {{unitMap.electricityUnit}}</div>
      </el-col>
      <el-col :span="6">
        <div>月竞交易总电量:{{totalMonthBidElectricity}} {{unitMap.electricityUnit}}</div>
      </el-col>
      <el-col :span="6">
        <div>跨区月竞交易电量占比:{{totalMonthBidOutprovProportion|keepTwoNum}} %</div>
      </el-col>
      <el-col :span="6">
      ...
      >
        <template slot-scope="scope">
          <el-select
            ...
            :clearable="true"
          >
            <!-- :disabled="isEdit || scope.row.tradingClassificationCode!='jyfl_02'" -->
            <el-option
              v-for="(item,index) in connectingPassageList"
              :key="index"
              :label="item.propName"
              :value="item.propCode"
            ></el-option>
...
<script>
import { getTransactionLine } from "@common/api/business/jySellNotice/index";
  ...
  data() {
    return {
      ...
      // distributionBtn: true,
       tabHeight:200,
      connectingPassageList: [],
      distributionData: "",
      totalMonthBidElectricity: "",
      totalMonthBidOutprovElectricity: "",
      totalMonthBidOutprovProportion: "",
      numberArray: [17, 17, 17, 17, 17, 15],
  ...
  methods: {
    ...
    init() {
      ...
      getTitleData(parme).then(res => {
        ...
        this.totalMonthBidOutprovElectricity =
          res.totalMonthBidOutprovElectricity; //省内交易电量
        if (
          res.totalMonthBidOutprovProportion == undefined ||
          res.totalMonthBidOutprovProportion == ""
        ) {
          this.totalMonthBidOutprovProportion = "";
        } else {
          this.totalMonthBidOutprovProportion =
            res.totalMonthBidOutprovProportion * 100; //月竞省内占比
        }
      });
    ...
    getTransactionLine() {
      getTransactionLine().then(res => {
        this.connectingPassageList = res;
      });
    },
    ...
    checkTab(data) {
      ...
      if (!this.isErr) {
        // if (Number(this.totalMonthBidOutprovElectricity) != totalNumber) {
        //   this.isErrInfo = "申报电量总合不等于省外交易电量";
        //   this.isErr = true;
        // }
      }
    },
      ...
      for (let key in this.form) {
        if (key.indexOf("Name") < 0) {
          if (key == "quotationStall") {
            newData["quotationStall"] = query.propCode;
            // newData[key + "Status"] = "";
          ...
          if (key == "quotationStallName") {
            newData["quotationStallName"] = query.propName;
          } else {
            newData[key] = "";
          }
        }
