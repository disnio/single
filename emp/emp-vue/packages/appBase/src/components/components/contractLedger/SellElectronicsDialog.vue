 ...
 * @最新修改内容: #2604 合同管理--合同台账管理--测试bug
 * @LastEditTime: 2021-03-12 17:24:14
 -->
<template>
  <div>
    <el-dialog
            ...
            >
              <div>
                <el-collapse v-model="activeNames">
                  ...
                  >
                    <earning
                      :sdTemplateName="sdTemplateName"
                      :unitMap="unitMap"
                      :proceedInfo="proceedInfo"
                      :ht="contractLedgerForm.htContract"
                      :cPrice="contractLedgerForm.clausePrice"
                      :clausePlanSource="contractLedgerForm.clausePlan"
                      :showGovernmentSubsidies="showGovernmentSubsidies"
                      :governmentOption="governmentOption"
                      :months="months"
                      :noSubsidy="noSubsidy"
                      :state="state"
                      :fkzq="fkzq"
                      ...
                      @shareYear="changeShareYear"
                      @shareMonth="changeShareMonth"
                      @change:ht="setErHt"
                      @change:cprice="setErClausePrice"
                      @change:cplan="setErClausePlan"
                      @change:swich="switchRule"
                    ></earning>
                  </el-collapse-item>
                  <!-- 收益方式 end -->
                  <!-- 用电计划  -->
                  <el-collapse-item
                    v-if="!proceedInfo.cxdl"
                    v-show="proceedInfo.LyydAzdl"
                    title="用电计划"
                    name="5"
                    style="margin-left:10px"
                  >
                    <clause-tb
                      :type="!proceedInfo.cxdl"
                      :clausePlanSource="contractLedgerForm.clausePlan"
                      :months="months"
                      :unitMap="unitMap"
                      :yearBilateElectricState="yearBilateElectricState"
                      :electricityTradeState="electricityTradeState"
                      @change:clause="changeClause"
                    ></clause-tb>
                  </el-collapse-item>
                  <!-- 用电计划 end -->
                  <!-- 用电计划  -->
...
import sd from '@common/components/contractLedger/sd';
import clauseTb from '@common/components/contractLedger/components/clauseTb';
import earning from '@common/components/contractLedger/components/earning';
...
export default {
    ...
    return {
      ...
      contractLedgerForm: {
        ...
        additionalClause: {
          ...
          valueServicesText: '', //增值服务条款
        },
        //#4083 gaoaining 2020-05027增值服务条款
        // 偏差
        clauseAssessment: {
          violationId: '',
  ...
  methods: {
    ...
    initDialog(dataInfo, dialogInfo) {
      this.getFawuIs();
      console.log('inOldInitDialog:', dataInfo, dialogInfo);
      ...
      let cols = _.keys(dataInfo.secondPartyInfo);
      let customerCols = _.map(cols, (v) => _.upperFirst(v));
      customerCols.forEach((k, i) => {
        this.contractLedgerForm.htContract[`sp${k}`] =
          dataInfo.secondPartyInfo[cols[i]];
      });

      // this.contractLedgerForm.htContract.secondPartName = dataInfo.secondPartName;
      // this.contractLedgerForm.htContract.secondPartCode = dataInfo.secondPartCode;
    ...
    initEditDialog(dataInfo, dialogInfo) {
      ...
      this.getFawuIs();
      console.log('initOldEditDialog Fw', dataInfo, dialogInfo);
    ...
    getcontractType() {
      ...
      getSelectList('payment_period').then((res) => {
        this.fkzq = res;
      });
    },
    // todo 验证计划
    checkTotal(total, key) {
      ...
      //#3626 gaoaining 2020-02-03 修改不填写总电量时 填写了用电计划中的年度长协分解电量、
      // 月度竞价分解电量时，在保存/提交时，自动计算填写总电量。
      // 总电量=各个月的年度长协分解电量和月度竞价分解电量的总和。
      if (this.contractLedgerForm.htContract.totalElectricity == '') {
        state = true;
        this.contractLedgerForm.htContract.totalElectricity = totalTemp;
      } else {
    ...
    // 提交保存
    //#3644 gaoaining 2020-02-05 修改总电量不填时   用电计划填写时可提交  start
    submitSave(dataStatus, type) {
      ...
      console.log('submitSave', dataStatus, type);
      if (
        ...
        if (
          ...
          if (this.contractLedgerForm.htContract.totalElectricity == '') {
            isYearBilateElectricSum = true;
            this.contractLedgerForm.htContract.totalElectricity = yearBilateElectric;
          } else {
            if (
              totalElectricity > yearBilateElectric ||
              totalElectricity < yearBilateElectric
            ) {
              console.log(totalElectricity, yearBilateElectric);
              isYearBilateElectricSum = false;
            }
          }
        } else if (
          ...
          if (this.contractLedgerForm.htContract.totalElectricity == '') {
            isElectricityTradeSum = true;
            this.contractLedgerForm.htContract.totalElectricity = electricityTrade;
          } else {
            if (
      ...
      //#3644 gaoaining 2020-02-05 修改总电量不填时   用电计划填写时可提交  end
      ...
      if (result) {
        ...
        if (this.contractLedgerForm.clausePrice.yearBidElectric != '') {
          let fwfTotal = Number(
    ...
    editObject() {
      ...
      // wsc 2020-10-13 混入法务系统必填字段
      parame.htContract = {
        ...parame.htContract,
        ...this.contractLedgerForm.legal,
      };
      console.log('edit ??', parame.htContract, this.contractLedgerForm.legal);
    ...
    handleClose() {
      ...
      this.contractLedgerForm = this.resetBdFields(this.contractLedgerForm);
      this.addDialogVisible = false;
    },
    ...
    switchRule(val) {
      ...
      if (this.contractLedgerForm.htContract.contractPriceWay === 'bjfs01') {
        obj[0].required = false;

        this.contractLedgerFormRules['clausePrice.kwhCost'] = obj;
      } else if (
        this.contractLedgerForm.htContract.contractPriceWay === 'bjfs02'
      ) {
        obj[0].required = true;
        this.contractLedgerFormRules['clausePrice.kwhCost'] = obj;
      }
